({
    ...
    doInit : function(component, event, helper) {
        ...
        component.set('v.visit',{'Status__c':'','Planned_Start_Time__c':'','EId__c':'','Revisit__c':'','AccountName__c':'','Account1__r':{"attributes":{"type":"Account"},"EId__c": ''}});
        ...
        component.set('v.Order',{'Order_Status__c':'' ,'EID__c':'','Grand_Total_Edit__c':'','Store__r':{"attributes":{"type":"Account"},"EID__c": ''},'Visit__r':{"attributes":{"type":"Visit__c"},"EID__c": ''}});
        component.set('v.Order_Item__c',{'Product__c':'' ,'EID__c':'','Quantity__c':'','Product_Name__c':'','Unit_Price__c':'','Total_Edit__c':'','TAX_Per_GST__c':'','Tax_value_edit__c':'','Total_Amount__c':'','Order__r':{"attributes":{"type":"Order__c"},"EID__c": ''},'Store__r':{"attributes":{"type":"Account"},"EID__c": ''},'Visit__r':{"attributes":{"type":"Visit__c"},"EID__c": ''}});
    ...
    createVisit: function (component, event, helper) {
        ...
        if(name != null && name != ''){
            ...
            var vistExist = false;
            for(var i=0;i<visits.length; i++){ 
                if(visits[i].Account1__r.EId__c == name){
                    vistExist = true;
                    break;
                } 
            } 
        ...
        }else if(vistExist == true && dateValue==today){
    ...
    var accounts = component.get('v.data.accounts') || [];
    var searchText = (component.get('v.searchText') || '').trim();
    var visitType = component.get('v.visit.Visit_Type__c');

    var matchaccounts = [];

    // Map visit type -> allowed Account_Type__c values
    // CHANGE THESE VALUES to your actual picklist values
    var visitTypeToAccountTypes = {
        "Doctor": ["Doctor"],
        "Pharmaceutical": ["Pharmaceutical"]
    };

    // If visitType has mapping, use it, else allow all
    var allowedTypes = visitTypeToAccountTypes[visitType] || null;

    if (searchText !== '') {
        for (var i = 0; i < accounts.length; i++) {
            var acc = accounts[i];
            var name = (acc.Name || '').toLowerCase();
            var type = acc.Account_Type__c; // <-- your field

            // 1) Account Type filter
            if (allowedTypes && allowedTypes.indexOf(type) === -1) continue;

            // 2) Name filter
            if (name.indexOf(searchText.toLowerCase()) !== -1) {
                matchaccounts.push(acc);
                if (matchaccounts.length >= 50) break;
            }
        }
    ...
    visiForChange : function (component, event, helper) {
        var visitType = component.get('v.visit.Visit_Type__c');
    ...
    orderClick: function (component, event, helper) {
        ...
        var visitId = component.get('v.data.currentvisit.EId__c');
         //alert(3)
        component.set('v.Order.Store__r.EID__c',storeId);
        component.set('v.Order.Visit__r.EID__c',visitId);
         component.set('v.Order.EID__c',eid);
        //component.set('v.Order.Order_Date__c',d);
        //alert(3.1);
    ...
    save: function(component,event,helper) {
        ...
        if (helper.validateOrderList(component, event)) {
            ...
            var dataOrderItems = component.get('v.data.orderitems');
             //alert(JSON.stringify(dataOrderItems));
    ...
    update2: function(component, event, helper) {
        ...
        for(var i=0;i<prds.length; i++){ 
            if(prds[i].Id === pid ){
                ...
                oitems[index].Unit_Price__c = prds[i].Distributive_Price_10__c;
                oitems[index].Minimum_Stock__c = prds[i].Minimum_Quantity__c;
                oitems[index].TAX_Per_GST__c = prds[i].TAX_Per_GST__c;
    ...
    getGrandTotal : function(component, event, helper) {
        ...
        console.log('up: '+oitems[index].Unit_Price__c);
        console.log('Qty: '+oitems[index].Quantity__c);
        console.log('tax: '+oitems[index].TAX_Per_GST__c);
        oitems[index].Total_Amount__c  = (((oitems[index].Unit_Price__c * oitems[index].Quantity__c)*oitems[index].TAX_Per_GST__c)/100)+(oitems[index].Unit_Price__c * oitems[index].Quantity__c);
        //alert(oitems[index].Total_Amount__c);
        component.set('v.orderItemList',oitems);
        var grandtotal = 0;
        var oit= component.get('v.orderItemList');
        for(var i=0;i<oit.length; i++){ 
            grandtotal = grandtotal+oit[i].Total_Amount__c;
        } 
        component.set('v.GrandTotal',grandtotal.toFixed(2));
         component.set('v.Order.Grand_Total_Edit__c',grandtotal.toFixed(2));
    ...
    getCheckout1 : function (component, event, helper) { 
        component.set('v.showkit', true);
        component.set("v.visitsView",false);
        component.set('v.showsurgery', false);
    },
     getCheckout2 : function (component, event, helper) { 
        component.set('v.showsurgery', true);
        component.set('v.showkit', false);
        component.set("v.visitsView",false);
    },
    
    createStoreStock: function(component, event, helper) {
    component.set('v.showStockEntry', true);
    component.set("v.visitsView", false);
    helper.addStockRecord(component, event);
},
    createStoreOrder: function(component, event, helper) {
    component.set('v.showorderEntry', true);
    component.set("v.visitsView", false);
    helper.addOrderRecord(component, event);
},

cancelStock: function(component, event, helper) {
    component.set("v.stockList", []);
    component.set("v.matchproductsStock", []);
    component.set("v.searchTextStock", "");
    component.set('v.showStockEntry', false);
    component.set("v.visitsView", true);
},
    
    cancelOrder: function(component, event, helper) {
    component.set("v.orderLists", []);
    component.set("v.matchproductsStock", []);
    component.set("v.searchTextOrder", "");
    component.set('v.showorderEntry', false);
    component.set("v.visitsView", true);
},
...
saveOrder: function(component, event, helper) {
    if(helper.validateOrderList(component, event)) {
        component.set('v.spinner', true);
        helper.saveOrderEntry(component, event, helper);
    }
},
    ...
    if(helper.validateStockList(component, event)) {
        component.set('v.spinner', true);
        helper.saveStockEntry(component, event, helper);
    }
},

addStockRow: function(component, event, helper) {
    helper.addStockRecord(component, event);
},

removeStockRow: function(component, event, helper) {
    ...
    addOrderRow: function(component, event, helper) {
    helper.addOrderRecord(component, event);
},

removeOrderRow: function(component, event, helper) {
    var stockList = component.get("v.orderLists");
    var selectedItem = event.currentTarget;
    var index = selectedItem.dataset.record;
    
    stockList.splice(index, 1);
    component.set("v.orderLists", stockList);
    
    if(stockList.length < 1) {
        helper.addOrderRecord(component, event);
    }
},
searchTextStock: function(component, event, helper) {
    var products = component.get('v.data.products');
    var searchText = component.get('v.searchTextStock');
    var matchproducts = [];
    ...
    if(searchText != '') {
        ...
        if(matchproducts.length > 0) {
            component.set('v.matchproductsStock', matchproducts);
        }
    } else {
        component.set('v.matchproductsStock', []);
    }
...
selectProductStock: function(component, event, helper) {
    var index = event.currentTarget.dataset.record;
    var pid = event.currentTarget.dataset.id;
    var prds = component.get('v.matchproductsStock');
    var stockList = component.get('v.stockList');
    ...
    searchTextOrder: function(component, event, helper) {
    var products = component.get('v.data.products');
    var searchText = component.get('v.searchTextOrder');
    var matchproducts = [];
...
selectProductOrder: function(component, event, helper) {
    var index = event.currentTarget.dataset.record;
    var pid = event.currentTarget.dataset.id;
    var prds = component.get('v.matchproductsStock');
    var stockList = component.get('v.orderLists');
