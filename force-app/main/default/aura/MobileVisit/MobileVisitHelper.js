({  
    ...
    dataHandler : function(component,event,helper) {  
        ...
        if(window.navigator.onLine ){  
            ...
            action12.setCallback(this,function(response){
                if(response.getState() == "SUCCESS"){ 
                     //alert('orderItem success');
                    helper.data.orderitems = response.getReturnValue();
                    component.set('v.data',  helper.data)
                    localStorage.setItem("data",   JSON.stringify(helper.data)   );
                }else if (response.getState() === "ERROR") {
					 //alert('orderItem error');
                var errors = response.getError();
                //alert(JSON.stringify(errors));
                if (errors) {
                    if (errors[0] && errors[0].message) {
    ...
    addProductRecord: function(component, event,visitId,StoreId) {
        ...
        productList.push({
            ...
            'Total_Amount__c':'',
            'Product_Name__c':'',
            'Store__r':{"attributes":{"type":"Account"},"EID__c": StoreId},
            'Visit__r':{"attributes":{"type":"Visit__c"},"EID__c": visitId},
            'Order__r':{"attributes":{"type":"Order__c"},"EID__c": component.get('v.Order.EID__c')},
    ...
    saveOrder : function(component,event,helper) {  
        ...
        var data =   JSON.parse( JSON.stringify(component.get('v.data') ) );
        //alert(JSON.stringify(data.orders));
        //alert(JSON.stringify(data.orderitems));
        if(!window.navigator.onLine){
    ...
    addOrderRecord: function(component, event) {
    var stockList = component.get("v.stockList");
    var visitId = component.get("v.data.currentvisit.Id");
    var accountId = component.get("v.data.currentvisit.Account1__r.Id");
...
saveStockEntry: function(component, event, helper) {
    var stockList = component.get("v.stockList");
    ...
    action.setCallback(this, function(response) {
        ...
        if(response.getState() == "SUCCESS") {
            helper.showToast('Stock entry saved successfully', 'Success');
    ...
    for(var i = 0; i < stockList.length; i++) {
        if(stockList[i].Product__c == '' || stockList[i].Product__c == null) {
            ...
            this.showToast('Product Name cannot be blank on row number ' + (i + 1), 'Warning');
            break;
        } else if(stockList[i].Quantity__c == '' || 
                 stockList[i].Quantity__c == null || 
                 stockList[i].Quantity__c < 0) {
            isValid = false;
            this.showToast('Please enter valid quantity on row number ' + (i + 1), 'Warning');
            break;
        }
    ...
    saveOrderEntry: function(component, event, helper) {
    var stockList = component.get("v.orderLists");
    
    var action = component.get("c.upsertOrderEntry");
    action.setParams({
        'stockList': JSON.stringify(stockList),
        'visitId' : component.get("v.data.currentvisit.Id"),
        'accountId' : component.get("v.data.currentvisit.Account1__r.Id")
    });
    ...
    action.setCallback(this, function(response) {
        ...
        if(response.getState() == "SUCCESS") {
            helper.showToast('Order entry saved successfully', 'Success');
