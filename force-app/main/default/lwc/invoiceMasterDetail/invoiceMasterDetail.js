...
export default class masterDetail extends NavigationMixin(LightningElement){
    ...
    @api   records  =  {'accounts':[], 'orders':[], 'order_items':[],'invoices':[], 'invoice_items':[],'visits' : [] ,'products':[]};
    @track record={ "Store__c":null, "Store__r":{ "Name":"" } ,"EID__c":""};
    @track items = [];  
    ...
    parseRecords(){ 
            this.record =  this.records.invoices.find(data => data.EID__c  === this.EID__c);
            this.items =   this.records.invoice_items.find(  data  => {         data.EID__c.startsWith(this.EID__c )       }  ) ;
        ...
        .then((result) => {
            if(result != null){
                console.log(result);  
                 ...
                 let items = JSON.parse(JSON.stringify( result.invoice_items )) ;
                 let record = JSON.parse(JSON.stringify( result.invoices.find( data  =>  data.Id  === this.recordId ) ));
    ...
    saveRecordsCall() {  
        this.records.invoice_items = this.items;
        this.records.invoices.push(  this.record ) ;

        for(var i=0;i<this.records.invoice_items.length;i++){
    ...
    handleRecordInputLookupChange(event) {
        ...
        if(event.detail ){  
            ...
            this.record.Store__c =   selectedRecord.Id; 
            console.log(JSON.stringify( event) );
